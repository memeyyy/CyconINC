...
<?php
}?> <a class="btn btn-primary btn-lg float-right" data-toggle="modal" data-target="#modalprint_trans">Print</a>
 <br><br><br>
                            <!--Table-->
                            <table id="example" class="table table-striped table-bordered table-responsive-md" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    <tr>
                                        <?php 
        $po = mysqli_query($connection,"SELECT * FROM `project_monitoring` WHERE visibility = 1 ORDER BY `project_monitoring`.`date_added` DESC");

        while ($po_data = mysqli_fetch_array($po)) 
        {
        $po1 = mysqli_query($connection,"SELECT pm.proj_Title,po.po_ORNum,po.po_Date,sum(bodq.boqdet_Amount) boqdet_Amount,s.status_Name,po.po_ID,pm.proj_ID,pm.proj_DateStarted,pm.proj_DateEnded,(SELECT SUM(pt_amount) FROM `purchase_transaction` WHERE term_ID =1  AND status_ID = 2 AND po_ID = po.po_ID) term_1_amount,(SELECT SUM(pt_amount) FROM `purchase_transaction` WHERE term_ID =2 AND status_ID = 2 AND po_ID = po.po_ID) term_2_amount,(SELECT SUM(pt_amount) FROM `purchase_transaction` WHERE term_ID =3 AND status_ID = 2 AND po_ID = po.po_ID) term_3_amount  FROM `purchase_order` po
...
LEFT join boq_detail bodq ON bodq.proj_ID = pm.proj_ID WHERE pm.proj_ID =  $po_data[0]");
        ...
        ?>
                                        ...
                                        <td>
                                            <?php echo number_format($po_data1[3],2);
                                            mysqli_query($connection,"UPDATE `project_monitoring` SET `proj_Costing` = '".$po_data1[3]."' WHERE `project_monitoring`.`proj_ID` = ".$po_data[0]);
                                        ...
                                        <td>
                                            <?php
                                             if ($po_data1[4] == "Ongoing") {
                                                 echo "with balance";
                                             }
                                             else if($po_data1[4] == "Complete"){
                                                echo "Paid";
                                             }
                                            else{
                                                echo "boq process";
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content " style="min-height: 500px;">
            <div class="modal-header text-center">
                <h5 class="modal-title w-100 font-bold">Bill of Quantity</h5>
...
<div class="modal fade" id="modalprint_trans" tabindex="-1" role="dialog" aria-labelledby="myModalLabel2" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header text-center">
                <h5 class="modal-title w-100 font-bold">Transaction Document</h5>
            ...
            <div class="modal-body mx-3">
                <center>
                    <form method="post" action="fpdf181/index.php" target="_blank">
                        <input class="btn btn-primary" type="submit" name="Print_All_Trans">
                    </form>
                </center>
