...
//Patrick Benny script Fit text to cell end
	...
	function Header()
		...
		if (empty($date)) 
			...
			{
				if ($type == 1) {
	        	$this->Cell(0,30, "Year: ".$date,0,0,'C');
	        	}
	        	else{
	        	$this->Cell(0,30, date("M , Y", strtotime($date)),0,0,'C');
	        	}
...
LEFT join boq_detail bodq ON bodq.proj_ID = pm.proj_ID WHERE pm.proj_ID =  $po_data[0]");
	...
	$pdf->CellFitSpace(50,5,$po_data1[0],0,0);
...
if (isset($_POST['Print_Sp'])) {
	...
	$filter_date = " WHERE date_added LIKE '%$date%'";
...
INNER JOIN `status` s ON pm.status_ID = s.status_ID
	...
	$pdf->CellFitScale(100,5,$data['proj_Title'],0,0);
