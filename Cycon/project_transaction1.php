                        ...
                        <div class="tab-pane card  active" id="all" role="tabpanel">
                            ...
                            <table id="example" class="table table-striped table-bordered table-responsive-md" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    <tr>
                                        <?php 
        $po = mysqli_query($connection,"SELECT * FROM `project_monitoring`");

        while ($po_data = mysqli_fetch_array($po)) 
        {
        $po1 = mysqli_query($connection,"SELECT pm.proj_Title,po.po_ORNum,po.po_Date,sum(bodq.boqdet_Amount) boqdet_Amount,s.status_Name,po.po_ID,pm.proj_ID  FROM `purchase_order` po
        ...
        <script>
            ...
            $(document).ready(function() {
                $(document).on('click', '#viewProjectOrder', function(e) {
                    ...
                    $.ajax({
                            url: 'project_transaction_View1.php',
                            type: 'POST',
                            data: 'id=' + uid,
                            dataType: 'html'
                        })
